                    ...
                    switch (value.sType) {
                        ...
                        case 'numeric':
                            ...
                            if (columnFilter !== null && columnFilter !== "" && columnFilter !== "~") {
                                ...
                                if (asRanges.length == 1) {
                                    //if not range then filter with "equal to '=' " 
                                    asColumnFilters.push("(" + sFieldName + " eq " + fnFormatValue(asRanges[0]) + ")");
                                }
                                else {
                                    if (asRanges[0] !== "" && asRanges[0] !== undefined) {
                                        //change from ">" to ">="
                                        asColumnFilters.push("(" + sFieldName + " ge " + fnFormatValue(asRanges[0]) + ")");
                                    }

                                    if (asRanges[1] !== "" && asRanges[1] !== undefined) {
                                        //change from "<" to "<="
                                        asColumnFilters.push("(" + sFieldName + " le " + fnFormatValue(asRanges[1]) + ")");
                                    }
                                }
                            }
                            break;
