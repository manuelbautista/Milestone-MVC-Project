...
(function ($) {
    ...
    $.fn.columnFilter = function (options) {
        ...
        function fnCreateDateRangeInput(oTable) {
            ...
            from.datepicker({ format: Globalize.culture().calendar.patterns.d.toLocaleLowerCase(), autoclose: true });
            //th.append(from);
            //th.append(_fnRangeLabelPart(1));
            var sToId = oTable.attr("id") + '_range_to_' + i;
            ...
            th.wrapInner('<div class="input-daterange input-group" />');
            to.datepicker({ format: Globalize.culture().calendar.patterns.d.toLocaleLowerCase(), autoclose: true });
            var index = i;
            aiCustomSearch_Indexes.push(i);
	        ...
	        function (oSettings, aData, iDataIndex) {
	            ...
	            var dStartDate = from.val() == "" ? null : from.datepicker("getDate");

	            var dEndDate = to.val() == "" ? null : to.datepicker("getDate");
	            ...
	            try {
	                if (aData[_fnColumnIndex(index)] == null || aData[_fnColumnIndex(index)] == "")
	                    return false;
	                dCellDate = Globalize.parseDate(aData[_fnColumnIndex(index)], Globalize.culture().calendar.patterns.d);
	            } catch (ex) {
	                return false;
	            }
	            if (dCellDate == null)
        ...
        var defaults = {
            ...
            iFilteringDelay: 500,
            aoColumns: null,
            sRangeFormat: "<span class='input-group-addon'>From</span> {from} <span class='input-group-addon'>To</span> {to}",
            sDateFromToken: "from",
            sDateToToken: "to"
        };
